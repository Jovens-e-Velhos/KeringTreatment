...
let DATA={};
let tModes={balenciaga:'all',bottega:'all',ysl:'all',gucci:'all'};
...
// ── FILTER ───────────────────────────────────────────────────
function getRows(brand){
  let rs=ALL_ROWS.filter(r=>r[0]===brand);
  if(fFrom||fTo){
...
// ── MODAL ────────────────────────────────────────────────────
const BNAMES={balenciaga:'Balenciaga',bottega:'Bottega Veneta',ysl:'Saint Laurent',gucci:'Gucci'};
...
// ── PILLS ────────────────────────────────────────────────────
['balenciaga','bottega','ysl','gucci'].forEach(brand=>{
...
def montar_html(rows, timestamp_str):
...
def main():
    ...
    html = montar_html(rows, timestamp_str)
